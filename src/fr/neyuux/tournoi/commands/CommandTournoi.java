package fr.neyuux.tournoi.commands;

import fr.neyuux.tournoi.Index;
import fr.neyuux.tournoi.enums.FightType;
...
public class CommandTournoi implements CommandExecutor {
	...
	public CommandTournoi(Index main) {
		this.main = main;
		file = main.getFile();
									...
									if (args.length > 2) {
										...
										for (String a : args)
											if (!a.equals(args[0]) && !a.equals(args[1]))
												if (Pays.getByCode(a) != null)
													pays.add(Pays.getByCode(a).ordinal());
										...
										try {
											...
											for (Integer i : pays)
												player.sendMessage(Pays.values()[i].getDisplayName());
										} catch (IOException e) {
								...
								if (args[1].equalsIgnoreCase("groups")) {
									if (args.length > 2)
										if (StringUtils.isNumeric(args[2])) {
											if (args.length >= 5) {
												if (Pays.getByCode(args[4]) != null && Pays.getByCode(args[5]) != null) {
													Pays pays1 = Pays.getByCode(args[4]);
													Player p1 = null;
													Pays pays2 = Pays.getByCode(args[5]);
													Player p2 = null;
													int bo;
													try {
													...
													for (String cs : yconfig.getConfigurationSection("players").getKeys(false))
														if (yconfig.getConfigurationSection("players").getInt(cs + ".pays") == pays1.ordinal())
															p1 = Bukkit.getPlayer(UUID.fromString(cs));
														else if (yconfig.getConfigurationSection("players").getInt(cs + ".pays") == pays2.ordinal())
															p2 = Bukkit.getPlayer(UUID.fromString(cs));
													
													if (p1 == null || p2 == null) {
														Bukkit.broadcastMessage(main.getPrefix() + "§cImpossible de lancer le combat §r" + pays1.getDisplayName() + " §cvs §r" + pays2.getDisplayName() + " §ccar les deux joueurs ne sont pas connectés");
														return true;
													}
									...
									if (args.length > 3) {
										if (Pays.getByCode(args[3]) != null && Pays.getByCode(args[4]) != null) {
											Pays pays1 = Pays.getByCode(args[3]);
											Player p1 = null;
											Pays pays2 = Pays.getByCode(args[4]);
											Player p2 = null;
											int bo;
											try {
