package fr.neyuux.tournament.utils.versions;
...
public abstract class VersionUtils {
    ...
    public static VersionUtils getVersionUtils() {
        if (versionUtils == null) {
            ...
            switch (version) {
                ...
                case 0:
                    versionUtils = new VersionUtils_1_8();
                    Bukkit.getLogger().warning(TournamentPlugin.LOG_PREFIX + "Failed to detect server version! " + versionString + "?");
                    break;
                default:
    ...
    public void sendTitleToALl(String title, String subtitle, int fadeInTime, int showTime, int fadeOutTime) {
        Bukkit.getOnlinePlayers().forEach(player -> this.sendTitle(player, title, subtitle, fadeInTime, showTime, fadeOutTime));
    }
